...
//! ZX(H)-calculus.
//!
//! You will most likely only want to deal with the [`Element`] and [`Diagram`]
//! types.
//!
...
pub enum ZXError {
    ...
    KronDisjointKetKeys(HashSet<usize>, HashSet<usize>),

    /// `ketbra: bra keys must be disjoint in kron`
    #[error("ketbra: bra keys must be disjoint in kron;\nlhs: {0:?}\nrhs: {1:?}")]
    KronDisjointBraKeys(HashSet<usize>, HashSet<usize>),
...
impl State {
    ...
    pub fn to_ket(self, idx: usize, basis: Basis) -> Element {
        ...
        return Element { terms: ketbras };
    }

    /// Convert `self` to the appropriate basis as an [`Element`] representing
...
/// amplitude.
///
/// Mathematically this is an expression of the form
/// ```math
...
impl KetBra {
    ...
    pub fn to_basis(&self, basis: Basis) -> Element {
        ...
        let terms: Vec<KetBra>
            ...
            .collect();
        return Element { terms }.simplified();
    }
    ...
    /// operation fails.
    pub fn kron(&self, rhs: &KetBra) -> ZXResult<KetBra> {
        ...
        self_ket_keys.is_disjoint(&rhs_ket_keys).then_some(())
            .ok_or(ZXError::KronDisjointKetKeys(
        ...
        self_bra_keys.is_disjoint(&rhs_bra_keys).then_some(())
            .ok_or(ZXError::KronDisjointBraKeys(
    ...
    /// within `self` and `rhs`'s kets and bras must be disjoint, otherwise this
    /// operation fails.
    pub fn into_kron(self, rhs: KetBra) -> ZXResult<KetBra> {
        let self_ket_keys: HashSet<&usize> = self.ket.keys().collect();
        let rhs_ket_keys: HashSet<&usize> = rhs.ket.keys().collect();
        self_ket_keys.is_disjoint(&rhs_ket_keys).then_some(())
            .ok_or(ZXError::KronDisjointKetKeys(
    ...
    /// within `self` and `rhs`'s kets and bras must be disjoint, otherwise this
    /// operation fails.
    pub fn multikron<'a, I>(ketbras: I) -> ZXResult<KetBra>
    ...
    /// within `self` and `rhs`'s kets and bras must be disjoint, otherwise this
    /// operation fails.
    pub fn into_multikron<I>(ketbras: I) -> ZXResult<KetBra>
    ...
    pub fn dot(&self, rhs: &Self) -> ZXResult<Element> {
        ...
        if let Some((idx, _))
            = wire_counts.iter().find(|(_idx, count)| count > &&1)
        {
            return Err(ZXError::DotDuplicateBraKey(*idx));
        }
        ...
        let terms: Vec<KetBra>
            ...
            .collect::<ZXResult<Vec<KetBra>>>()?;
        return Ok(Element::new(terms)?.simplified());
    }
    ...
    /// ```
    /// The product must not leave multiple states on the same wire.
    pub fn into_dot(self, rhs: Self) -> ZXResult<Element> { self.dot(&rhs) }
}
...
impl IntoIterator for Element {
    ...
    fn into_iter(self) -> Self::IntoIter {
        return ElementIntoIter { iter: self.terms.into_iter() };
    }
}
...
impl Element {
    ...
    pub fn new<I>(ketbras: I) -> ZXResult<Self>
    where I: IntoIterator<Item = KetBra>
    {
        ...
        ketbras.iter()
            ...
            .then_some(())
            .ok_or(ZXError::ElementKetSameKeys)?;
    ...
    pub fn cap(ins: [usize; 2], phase: Option<f64>) -> Self {
        return Self::Z(ins, [], phase);
    }

    /// Alias for [`Self::new`] for semantics purposes.
    pub fn from_sum<I>(terms: I) -> ZXResult<Self>
    where I: IntoIterator<Item = KetBra>
    {
    ...
    /// within `self` and `rhs`'s kets and bras must be disjoint, otherwise this
    /// operation fails.
    pub fn kron(&self, rhs: &Self) -> ZXResult<Self> {
        let terms: Vec<KetBra>
            = Itertools::cartesian_product(
                self.terms.iter(),
                rhs.terms.iter(),
    ...
    /// within `self` and `rhs`'s kets and bras must be disjoint, otherwise this
    /// operation fails.
    pub fn multikron<'a, I>(elems: I) -> ZXResult<Self>
    ...
    /// within `self` and `rhs`'s kets and bras must be disjoint, otherwise this
    /// operation fails.
    pub fn into_multikron<I>(elems: I) -> ZXResult<Self>
    ...
    pub fn dot(&self, rhs: &Self) -> ZXResult<Self> {
        let terms: Vec<KetBra>
            = Itertools::cartesian_product(
                self.terms.iter(),
                rhs.terms.iter(),
    ...
    /// product. See also [`KetBra::dot`]. The product must not leave multiple
    /// states on the same wire.
    pub fn multidot<'a, I>(elems: I) -> ZXResult<Self>
    where I: IntoIterator<Item = &'a Element>
    {
    ...
    /// product. See also [`KetBra::dot`]. The product must not leave multiple
    /// states on the same wire.
    pub fn into_multidot<I>(elems: I) -> ZXResult<Self>
    where I: IntoIterator<Item = Element>
    {
        let mut acc = Self::one();
        for elem in elems.into_iter() {
...
impl IntoIterator for Diagram {
    ...
    fn into_iter(self) -> Self::IntoIter {
        return DiagramIntoIter { iter: self.slices.into_iter() };
    }
}
